  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([c (eval-under-env (call-funexp e) env)])
           (if (closure? c)
               (letrec ([formal-actual (cons (fun-formal (closure-fun c))
                                             (eval-under-env (call-actual e) env))]
                        [new-env (if (fun-nameopt (closure-fun c))
                                     (cons (cons (fun-nameopt (closure-fun c)) c)
                                           (cons formal-actual (closure-env c)))
                                     (cons formal-actual (closure-env c)))])
                 (eval-under-env (fun-body (closure-fun c)) new-env))
               (error "bad MUPL expression")))]       
...
(define mupl-map
  (fun "mupl-map" "fun" (fun "map-each" "mlist" (ifaunit (var "mlist")
                                                         (aunit)
                                                         (apair (call (var "fun") (fst (var "mlist")))
                                                                (call (var "map-each") (snd (var "mlist"))))))))

(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun "mupl-mapAddN" "int"
             (fun "add-each" "mlist" (call (call (var "map") (fun "add-n" "e" (add (var "int") (var "e"))))
                                           (var "mlist"))))))
