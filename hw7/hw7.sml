...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(r1, r2, s) => let val e = eval_prog (s, env)
			    in case e of
				   NoPoints => e
				 | Point(e1, e2) => Point(e1+r1, e2+r2)
				 | Line(e1, e2) => Line(e1, e2+r2-e1*r1)
				 | VerticalLine e1 => VerticalLine (e1+r1)
				 | LineSegment (e1,e2,e3,e4) => LineSegment (e1+r1,e2+r2,e3+r1,e4+r2)
			    end
(* CHANGE: Add a case for Shift expressions *)

(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)
fun preprocess_prog e = case e of
			    LineSegment (a, b, c, d) => if real_close(a,c) andalso real_close(b,d)
							 then Point (a,b)
							 else 
							     if (real_close(a,c)=false andalso a > c)
								orelse (real_close (a,c) andalso b > d)
